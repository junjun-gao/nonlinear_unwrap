...
# Arc height difference: about -40 ~ 40 m
HEIGHT_STDEV_M = 20.0

# Point linear velocity: -2 ~ 2 mm/year
# Arc linear velocity difference: about -4 ~ 4 mm/year
# MATLAB search range = +/- 2 * stdev
LINEAR_STDEV_M_PER_YEAR = 0.002

# Point quadratic coefficient: about -8 ~ -2 mm/year^2
# Arc quadratic coefficient difference: about -6 ~ 6 mm/year^2
# A small margin is reserved here: search range = +/- 8 mm/year^2
QUADRATIC_STDEV_M_PER_YEAR2 = 0.004
...
def unwrap_arcs_quadratic_periodogram(phase_wrapped, arcs, model, grids):
    ...
    for batch_id, start in enumerate(range(0, n_arcs, ARC_BATCH_SIZE)):
        ...
        #
        # MATLAB:
        # self(1:2).periodogram_estimation(..., nRefine=0)
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Step 2: remove height and estimate
        # linear + quadratic
        #
        # MATLAB:
        # self(2:end).periodogram_estimation(...)
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Step 3: fine 3-D joint search
        #
        # MATLAB:
        # finemodel stdev = original stdev / 10
        # ----------------------------------------------------
...
def main():
    ...
    print("linear arc search  : +/- {:.3f} mm/year | step {:.3f} mm/year".format(
        2.0 * LINEAR_STDEV_M_PER_YEAR * 1000.0,
        steps[1] * 1000.0,
    ))
    print("quadratic arc search: +/- {:.3f} mm/year^2 | step {:.3f} mm/year^2".format(
        2.0 * QUADRATIC_STDEV_M_PER_YEAR2 * 1000.0,
        steps[2] * 1000.0,
    ))
