...
MAX_ARC_DIST = None
PLOT_RANDOM_SEED = 42
OUTPUT_FOLDER = "known_height_seasonal_linear_unwrap_results"
...
def build_seasonal_linear_fit_operator(time_year, weight):
    ...
    design = np.column_stack((
        ...
        np.cos(annual_omega * time_relative) - 1.0,
    ))
...
def seasonal_linear_displacement_from_wrapped_phase(displacement_phase, design, fit_operator,
    ...
    phase_reference = displacement_phase[:, [0]]
    phase_relative_wrapped = wrap_phase(displacement_phase - phase_reference)

    phase_jumps = np.zeros_like(phase_relative_wrapped)
    phase_jumps[:, 1:] = wrap_phase(np.diff(phase_relative_wrapped, axis=1))
    phase_unwrapped = np.cumsum(phase_jumps, axis=1)

    parameters = np.dot(phase_unwrapped, fit_operator.T)
    phase_model_relative = np.dot(parameters, design.T)

    for _ in range(ITERATIVE_TIMES):
        ambiguity = np.rint((phase_model_relative - phase_relative_wrapped) / (2.0 * np.pi)).astype(np.int64)
        phase_unwrapped = phase_relative_wrapped + 2.0 * np.pi * ambiguity
        parameters = np.dot(phase_unwrapped, fit_operator.T)
        phase_model_relative = np.dot(parameters, design.T)

    phase_model = phase_reference + phase_model_relative
    return phase_model * wavelength / (displacement_sign * 4.0 * np.pi)

def unwrap_arcs_known_height(phase_wrapped, arcs, delta_h_true, bperp, time_year,
                             wavelength, incidence_angle_deg, slant_range):
    ...
    arc_delta_h = arc_delta_h_known.copy()
    ...
    for batch_id, start in enumerate(range(0, n_arcs, BATCH_SIZE)):
        ...
        h_known = arc_delta_h_known[start:end]
        known_height_phase = h_known[:, None] * height_system[None, :]
        displacement_phase = wrap_phase(obs - known_height_phase)
        ...
        temporal_model0 = known_height_phase + DISPLACEMENT_SIGN * 4.0 * np.pi * d_est / wavelength
...
def accuracy_report(df, h_est, deformation_m, true_def_rel, ref_idx, net, noise_std):
    ...
    summary["known_height_error_used"] = True
...
def main():
    ...
    print("================ Known-height seasonal-linear arc unwrapping ================")
