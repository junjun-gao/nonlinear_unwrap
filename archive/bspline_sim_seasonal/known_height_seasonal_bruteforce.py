...
OUTPUT_FOLDER = "known_height_seasonal_bruteforce_results"
MAX_ARC_DIST = None
ARC_BATCH_SIZE = 32
...
# Brute-force search range = +/- 2 * stdev
...
def locate_input_files():
    cwd = os.getcwd()
    candidate_dirs = [
        os.path.join(cwd, "observed_seasonal_phase_csv"),
        os.path.join(cwd, "observed_phase_csv"),
        cwd,
    ]
...
def build_seasonal_model(bperp, time_year, wavelength, incidence_angle_deg, slant_range):
    ...
    systems = np.column_stack((linear_system, seasonal_sin_system, seasonal_cos_system))
    displacement_basis = np.column_stack((centered_time, np.sin(2.0 * np.pi * centered_time), np.cos(2.0 * np.pi * centered_time)))
    stdev = np.asarray([LINEAR_STDEV_M, SEASONAL_STDEV_M, SEASONAL_STDEV_M], dtype=float)
...
def prepare_periodogram_grids(model):
    ...
    for zoom in range(FINE_N_REFINE + 1):
        ...
        grids["refine_{}".format(zoom)] = build_candidate_grid(
            systems,
            zoom_axes,
            "refine_{}".format(zoom),
        )
...
# ============================================================
# 14. Known-height seasonal brute-force arc unwrapping
# ============================================================

def unwrap_arcs_known_height_bruteforce(phase_wrapped, arcs, delta_h_true, model, grids):
    systems = model["systems"]
    height_system = model["height_system"]
    n_arcs = len(arcs)
    n_time = phase_wrapped.shape[1]

    delta_h_true = np.asarray(delta_h_true, dtype=float)
    if delta_h_true.ndim != 1 or delta_h_true.size != phase_wrapped.shape[0]:
        raise ValueError("delta_h_true must contain one known height error for each PS point.")
    if not np.all(np.isfinite(delta_h_true)):
        raise ValueError("delta_h_true contains non-finite values.")

    arc_delta_h_known = delta_h_true[arcs[:, 1]] - delta_h_true[arcs[:, 0]]
    ...
    for batch_id, start in enumerate(range(0, n_arcs, ARC_BATCH_SIZE)):
        ...
        # Remove the exactly known height-error phase before any deformation search.
        known_height_phase = arc_delta_h_known[start:end, None] * height_system[None, :]
        deformation_phase_wrapped = wrap_phase(obs - known_height_phase)

        # Joint brute-force search for linear, seasonal-sine and seasonal-cosine coefficients.
        deformation_model_phase, bestfit, _, _ = periodogram_estimation_block(
            deformation_phase_wrapped,
            systems,
            grids["coarse"],
            refine_grids,
        )
        modelphase = known_height_phase + deformation_model_phase
        ...
        arc_coherence[start:end] = np.abs(np.mean(np.exp(1j * residual), axis=1)).astype(np.float32)

        if batch_id == 0 or (batch_id + 1) % 10 == 0 or end == n_arcs:
            print("  known-height brute-force unwrap: {}/{} ({:.1f}%)".format(end, n_arcs, 100.0 * end / n_arcs))
    ...
    return {
        ...
        "arc_delta_h": arc_delta_h_known.astype(float),
...
def spatial_network_inversion(net, model, wavelength, known_height_error, ref_idx):
    known_height_error = np.asarray(known_height_error, dtype=float)
    n_points = known_height_error.size
    ...
    h_est = known_height_error - known_height_error[ref_idx]
...
def save_results(df, deformation_cols, net, point_params, h_est, deformation_m, ref_idx, output_dir, noise_std, model):
    ...
    if model["linear_unit"] == "m/stack":
        output_data["linear_est_mm_per_stack"] = point_params[:, 0] * 1000.0
    else:
        output_data["linear_est_mm_per_year"] = point_params[:, 0] * 1000.0

    output_data["seasonal_sin_est_mm"] = point_params[:, 1] * 1000.0
    output_data["seasonal_cos_est_mm"] = point_params[:, 2] * 1000.0
    output_data["seasonal_amplitude_est_mm"] = np.sqrt(point_params[:, 1] ** 2 + point_params[:, 2] ** 2) * 1000.0
    output_data["seasonal_phase_est_rad"] = np.arctan2(point_params[:, 2], point_params[:, 1])
    ...
    arc_data = {
        "p1": net["arcs"][:, 0],
        "p2": net["arcs"][:, 1],
        "arc_delta_h_known_m": net["arc_delta_h"],
        "arc_linear_est": arc_params[:, 0],
        "arc_seasonal_sin_est_m": arc_params[:, 1],
        "arc_seasonal_cos_est_m": arc_params[:, 2],
        "arc_seasonal_amplitude_est_m": np.sqrt(arc_params[:, 1] ** 2 + arc_params[:, 2] ** 2),
        "arc_seasonal_phase_est_rad": np.arctan2(arc_params[:, 2], arc_params[:, 1]),
...
def main():
    ...
    print("================ Known-height seasonal brute-force arc unwrapping ================")
    ...
    print("height correction   : known delta_h_true_m")
    print("linear arc search   : +/- {:.3f} mm/year | step {:.3f} mm/year".format(2.0 * LINEAR_STDEV_M * 1000.0, steps[0] * 1000.0))
    print("seasonal arc search : +/- {:.3f} mm | step {:.3f} mm".format(2.0 * SEASONAL_STDEV_M * 1000.0, steps[1] * 1000.0))
    ...
    for noise_std, observed_csv in observed_files:
        ...
        if "delta_h_true_m" not in df.columns:
            raise ValueError("{} does not contain the required delta_h_true_m column.".format(observed_csv))
        delta_h_true = df["delta_h_true_m"].values.astype(float)
        ...
        net = unwrap_arcs_known_height_bruteforce(
            phase_wrapped,
            arcs,
            delta_h_true,
            model,
