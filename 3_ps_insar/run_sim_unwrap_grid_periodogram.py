...
from grid_periodogram_temporal_unwrap import unwrap_arcs_grid_periodogram
...
def main():
    ...
    OUTPUT_CSV = os.path.join(
        OUTPUT_DIR,
        "simulated_unwrapped_grid_noise_0p2rad_result.csv"
    )
    ...
    GRID_PERIODOGRAM_PARAM = {
        "sentinel-1": {
            "wavelength": 0.056,  # m
            "incidence_angle": 35.0,  # degree

            # 沿用原始公式：
            # R = H / cos(incidence_angle)
            # 所以如果你想让 R = SLANT_RANGE，
            # H 应该写成：
            # H = SLANT_RANGE * cos(incidence_angle)
        ...
        # 如果你生成形变相位时是：
        # deformation_phase =  4*pi*d/lambda，设置为 1
        # deformation_phase = -4*pi*d/lambda，设置为 -1
        "displacement_sign": 1,
    }
    ...
    net = unwrap_arcs_grid_periodogram(
        phase_wrapped=data["phase_wrapped"],
        arcs=arcs,
        bperp=data["Bperp"],
        time_year=data["t_years"],
        param=GRID_PERIODOGRAM_PARAM,
        mode="grid-period",
        n_jobs=40,
        chunk_size=100
    )
