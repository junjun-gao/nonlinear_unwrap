...
CSV_FILE = "/data/tests/junjun/nonlinear_unwrap/simulation/model_free_simulation/bspline_arc_unwrap_results/unwrapped_deformation_noise_0p2rad.csv"
...
if ref_idx < 0 or ref_idx >= len(df): raise ValueError("ref_idx=%d 超出数据范围 0~%d" % (ref_idx, len(df) - 1))
...
# 第三幅图固定为 -4 ~ 4 mm
deformation_error_vmin = -4
deformation_error_vmax = 4
deformation_error_ticks = np.arange(-4, 5, 1)

# 第四幅图固定为 -3 ~ 3 m
height_error_vmin = -3
height_error_vmax = 3
height_error_ticks = np.arange(-3, 4, 1)

# ============================================================
# 通用二维绘图函数
# ============================================================

def plot_map(values, filename, title, cbar_label, vmin, vmax, mark_reference=False, mark_selected=False, cbar_ticks=None):
...
plot_map(
    ...
    deformation_error_vmin,
    deformation_error_vmax,
    mark_selected=True,
    cbar_ticks=deformation_error_ticks
)
