...
OUTPUT_FOLDER = "quadratic_periodogram_results"
MAX_ARC_DIST = None
ARC_BATCH_SIZE = 32
...
# Total span = 79 * 12 = 948 days = about 2.60 years
#
# If the whole 200 mm deformation is linear:
# point velocity ~= 200 / 2.60 = 77 mm/year
# worst-case arc difference ~= 154 mm/year
#
# Search range with margin:
# arc linear velocity = -200 ~ 200 mm/year
#
# MATLAB searchspace = +/- 2 * stdev
LINEAR_SEARCH_MAX_MM_PER_YEAR = 200.0
LINEAR_STDEV_M_PER_YEAR = LINEAR_SEARCH_MAX_MM_PER_YEAR / 2.0 / 1000.0

# If the whole 200 mm deformation is quadratic:
# d(t) = a*t^2
# point coefficient ~= 200 / 2.60^2 = 29.7 mm/year^2
# worst-case arc difference ~= 59.4 mm/year^2
#
# Search range with margin:
# arc quadratic coefficient = -80 ~ 80 mm/year^2
#
# MATLAB searchspace = +/- 2 * stdev
QUADRATIC_SEARCH_MAX_MM_PER_YEAR2 = 80.0
QUADRATIC_STDEV_M_PER_YEAR2 = QUADRATIC_SEARCH_MAX_MM_PER_YEAR2 / 2.0 / 1000.0
...
def build_quadratic_model(bperp, time_year, wavelength, incidence_angle_deg, slant_range):
    ...
    quadratic_system = m2ph * model_time ** 2

    systems = np.column_stack((height_system, linear_system, quadratic_system))
    displacement_basis = np.column_stack((model_time, model_time ** 2))
    stdev = np.asarray([HEIGHT_STDEV_M, LINEAR_STDEV_M_PER_YEAR, QUADRATIC_STDEV_M_PER_YEAR2], dtype=float)

    return {
        "systems": systems,
        "height_system": height_system,
        "linear_system": linear_system,
        "quadratic_system": quadratic_system,
...
# Step 2: linear + quadratic
# Step 3: height + linear + quadratic
# ============================================================
...
def prepare_periodogram_grids(model):
    ...
    steps = np.asarray([matlab_stepsize(systems[:, i]) for i in range(3)], dtype=float)
    coarse_axes = [matlab_searchspace(steps[i], stdev[i]) for i in range(3)]
    refine_axes = [matlab_refine_space(steps[i], 1.0) for i in range(3)]
    ...
    grids["stage2_coarse"] = build_candidate_grid(systems[:, 1:3], coarse_axes[1:3], "stage2_coarse")
    grids["stage2_refine"] = build_candidate_grid(systems[:, 1:3], refine_axes[1:3], "stage2_refine")

    fine_stdev = stdev / 10.0
    fine_coarse_axes = [matlab_searchspace(steps[i], fine_stdev[i]) for i in range(3)]
    ...
    for zoom in range(1, FINE_N_REFINE + 1):
        zoom_axes = [matlab_refine_space(steps[i], 0.1 ** zoom) for i in range(3)]
...
# ============================================================
# 14. Linear + quadratic periodogram 3-step arc unwrapping
# ============================================================

def unwrap_arcs_quadratic_periodogram(phase_wrapped, arcs, model, grids):
    systems = model["systems"]
    n_arcs = len(arcs)
    ...
    arc_params = np.zeros((n_arcs, 3), dtype=np.float32)
    ...
    for batch_id, start in enumerate(range(0, n_arcs, ARC_BATCH_SIZE)):
        ...
        _, fit_stage1, _, _ = periodogram_estimation_block(
            obs,
            systems[:, 0:2],
            grids["stage1_coarse"],
            [grids["stage1_refine"]],
        ...
        # ----------------------------------------------------
        # Step 3: fine height + linear + quadratic joint search
        # ----------------------------------------------------
        ...
        if batch_id == 0 or (batch_id + 1) % 10 == 0 or end == n_arcs:
            print("  quadratic periodogram: {}/{} ({:.1f}%)".format(end, n_arcs, 100.0 * end / n_arcs))
    ...
    return {
        ...
        "arc_quadratic_coefficient": arc_params[:, 2].astype(float),
...
def plot_deformation_comparison(df, deformation_cols, deformation_m, ref_idx, point_idx, time_year, noise_std, output_dir):
    ...
    if "quadratic_coefficient_mm_per_year2" in df.columns: print("Quadratic coefficient : {:.3f} mm/year^2".format(df["quadratic_coefficient_mm_per_year2"].iloc[point_idx]))
...
def save_results(df, deformation_cols, net, point_params, h_est, deformation_m, ref_idx, output_dir, noise_std):
    ...
    keep_cols = [
        ...
        "longitude",
        "latitude",
        "quadratic_coefficient_mm_per_year2",
    ...
    output_data["quadratic_est_mm_per_year2"] = point_params[:, 2] * 1000.0
    ...
    if "quadratic_coefficient_mm_per_year2" in df.columns:
        quadratic_true = df["quadratic_coefficient_mm_per_year2"].values.astype(float)
        quadratic_true_rel = quadratic_true - quadratic_true[ref_idx]
        output_data["quadratic_true_relative_mm_per_year2"] = quadratic_true_rel
        output_data["quadratic_error_mm_per_year2"] = point_params[:, 2] * 1000.0 - quadratic_true_rel
    ...
    arc_data = {
        ...
        "arc_quadratic_est_m_per_year2": arc_params[:, 2],
    ...
    if "quadratic_coefficient_mm_per_year2" in df.columns:
        quadratic_true_m = df["quadratic_coefficient_mm_per_year2"].values.astype(float) / 1000.0
        arc_quadratic_true = quadratic_true_m[net["arcs"][:, 1]] - quadratic_true_m[net["arcs"][:, 0]]
        arc_data["arc_quadratic_true_m_per_year2"] = arc_quadratic_true
        arc_data["arc_quadratic_error_m_per_year2"] = arc_params[:, 2] - arc_quadratic_true
...
def accuracy_report(df, h_est, deformation_m, true_def_rel, ref_idx, net, noise_std, steps):
    summary = {
    ...
    quadratic_limit = QUADRATIC_SEARCH_MAX_MM_PER_YEAR2 / 1000.0

    height_boundary_ratio = np.mean(np.abs(net["arc_params"][:, 0]) >= max(0.0, height_limit - steps[0]))
    linear_boundary_ratio = np.mean(np.abs(net["arc_params"][:, 1]) >= max(0.0, linear_limit - steps[1]))
    quadratic_boundary_ratio = np.mean(np.abs(net["arc_params"][:, 2]) >= max(0.0, quadratic_limit - steps[2]))
    ...
    print("quadratic-search boundary arcs: {:.2f}%".format(100.0 * quadratic_boundary_ratio))
    ...
    summary["quadratic_search_boundary_ratio"] = quadratic_boundary_ratio
...
def main():
    ...
    model = build_quadratic_model(bperp, time_year, wavelength, incidence_angle_deg, slant_range)
    ...
    equivalent_point_linear = MAX_CUMULATIVE_DEFORMATION_MM / total_years
    equivalent_arc_linear = 2.0 * equivalent_point_linear

    equivalent_point_quadratic = MAX_CUMULATIVE_DEFORMATION_MM / total_years ** 2
    equivalent_arc_quadratic = 2.0 * equivalent_point_quadratic

    print("================ Linear + quadratic periodogram arc unwrapping ================")
    ...
    print("time range          : {:.3f} years".format(total_years))
    print("time range          : {:.1f} days".format(total_days))
    print("max deformation     : {:.1f} mm".format(MAX_CUMULATIVE_DEFORMATION_MM))
    print("deformation model   : d(t) = v*t + a*t^2")

    print("\n================ Search range design ================")
    print("pure-linear point equivalent     : {:.3f} mm/year".format(equivalent_point_linear))
    print("pure-linear arc equivalent       : {:.3f} mm/year".format(equivalent_arc_linear))
    print("pure-quadratic point equivalent  : {:.3f} mm/year^2".format(equivalent_point_quadratic))
    print("pure-quadratic arc equivalent    : {:.3f} mm/year^2".format(equivalent_arc_quadratic))

    print("\n================ Actual search ranges ================")
    ...
    print("quadratic arc search: +/- {:.3f} mm/year^2 | step {:.3f} mm/year^2".format(QUADRATIC_SEARCH_MAX_MM_PER_YEAR2, steps[2] * 1000.0))
    print("refine points       : {} | fine zoom passes: {}".format(REFINE_POINTS, FINE_N_REFINE))

    print("\n================ Grid sizes ================")
    for name, grid in grids.items(): print("{:<20s}: {:d}".format(name, grid["n_candidates"]))
    ...
    for noise_std, observed_csv in observed_files:
        ...
        net = unwrap_arcs_quadratic_periodogram(phase_wrapped, arcs, model, grids)
